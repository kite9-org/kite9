<xsl:stylesheet
  ...
  <xsl:template name="labels-basic">
    ...
    <xsl:call-template name="formats-text-shape-inline">
      ...
      <xsl:with-param name="k9-ui">delete edit xml drag place fill stroke size font direction align</xsl:with-param>
    </xsl:call-template>
  ...
  <xsl:template name="labels-diagram-palettes">
     <adl:palette url="/public/templates/labels/palette.adl" />
  </xsl:template>  
