...
export function parseStyle(e) {	
	const PROPERTY = 0;
	const PROPERTY_MATCH = /^\s*([-a-zA-Z0-9]+)\s*/i

	const COLON = 1;
	const COLON_MATCH = /\s*:\s*/i
	
	const VALUE = 2;
	const VALUE_MATCH = /\s*(([A-Za-z0-9\-\+\.\(\)]+)|(".*")|('.*'))\s*(!important)?\s*/i
	
	const SEMICOLON = 5;
	const SEMICOLON_MATCH = /\s*;\s*/i
	...
	function consume(regex) {
		...
		if (result) {
			style = style.substring(result[0].length);
			return result[1] ? result[1] : true;
		}
	...
	if (style) {
		while (style.length > 0) {
			switch (mode) {
				case PROPERTY:
					propertyName = consume(PROPERTY_MATCH);
					if (propertyName) {
						mode = COLON;
						break;
					} else {
				...
				case VALUE:
					propertyValue = consume(VALUE_MATCH);
					if (propertyValue) {
