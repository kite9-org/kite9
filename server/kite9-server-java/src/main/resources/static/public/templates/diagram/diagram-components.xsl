<xsl:stylesheet
    ...
    <xsl:template name="diagram-root-svg" match="/">
        <xsl:param name="css"><xsl:apply-templates select="/" mode="diagram-css" /></xsl:param>
    ...
    <xsl:template name="diagram-basic">
      <xsl:call-template name="formats-container">
        ...
        <xsl:with-param name="rounding">0</xsl:with-param>
      </xsl:call-template>
    </xsl:template>
  ...
  <xsl:template match="text()" mode="diagram-css" />
