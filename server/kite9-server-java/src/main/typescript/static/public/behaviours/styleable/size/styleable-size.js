...
import { addNumericControl, moveContextMenuAway } from '../styleable.js';
...
export function initMarginsBuildControls() {
    return function (selectedElement, style, overlay, cm, event) {
        ...
        const innerMove = overlay.createSizingRect(bbox.x, bbox.y, bbox.width, bbox.height, 0, 0, 0, 0);
        const outerMove = overlay.createSizingRect(bbox.x, bbox.y, bbox.width, bbox.height, margins[0], margins[1], margins[2], margins[3]);
...
export function initPaddingBuildControls() {
    return function (selectedElement, style, overlay, cm, event) {
        ...
        overlay.createSizingRect(ibox.x, ibox.y, ibox.width, ibox.height, 0, 0, 0, 0);
        const outerMove = overlay.createSizingRect(ibox.x, ibox.y, ibox.width, ibox.height, padding[0], padding[1], padding[2], padding[3]);
        const numericControls = [
