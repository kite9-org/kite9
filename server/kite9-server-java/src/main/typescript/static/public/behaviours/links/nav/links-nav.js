 ...
 */
export function initLinksNavContextMenuCallback(singleSelect, selector = undefined) {
    if (selector == undefined) {
        selector = function () {
            return Array.from(getMainSvg().querySelectorAll("[id].selected"));
        };
    }
    function selectElements(elements, cm) {
    ...
    function highlight(elements, active) {
    ...
    return function (event, cm) {
        ...
        if (lastElement) {
            if (isTerminator(lastElement)) {
                ...
                cm.addControl(event, "/public/behaviours/links/link.svg", 'Link', () => selectElements([link], cm), 'Related', {
                ...
                if (isPort(portForElement)) {
                    cm.addControl(event, "/public/behaviours/links/nav/port.svg", 'Containing Port', () => selectElements([portForElement], cm), 'Related', {
                ...
                deps.forEach(element => {
                    cm.addControl(event, "/public/behaviours/links/nav/ends.svg", 'Terminator', () => selectElements([element], cm), 'Related', {
                ...
                const deps = getDependentElements([lastElement.getAttribute("id")])
                    .sort(leftToRightSort);
                deps.forEach(dep => {
                    cm.addControl(event, "/public/behaviours/links/link.svg", 'Link', () => selectElements([dep], cm), 'Related', {
                        "onmouseover": () => highlight([dep], true),
                        "onmouseout": () => highlight([dep], false)
                    });
                });
                if (deps.length > 1) {
                    cm.addControl(event, "/public/behaviours/links/link.svg", 'Link', () => selectElements(deps, cm), 'Related', {
