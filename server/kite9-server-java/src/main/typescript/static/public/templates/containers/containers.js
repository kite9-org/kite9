...
function initContainers() {
    if (metadata.isEditor()) {
        ...
        dragger.dropWith(initContainmentDropCallback(command, initBiFilter(containment, ['connected'], ['connected', 'diagram'])));
        ...
        const margins = initStyleContextMenuCallback(command, overlay, marginsIcon, 'Margins', initMarginsBuildControls(), containerSizingSelector);
        const padding = initStyleContextMenuCallback(command, overlay, paddingIcon, 'Padding', initPaddingBuildControls(), containerSizingSelector);
        const minSize = initStyleContextMenuCallback(command, overlay, minSizeIcon, 'Minimum Size', initMinSizeBuildControls(), containerSizingSelector);
        const sizing = initStyleContextMenuCallback(command, overlay, sizingIcon, 'Sizing Rules', initBasicBuildControls('Sizing', sizingEnumProperties, sizingEnumValues), containerSizingSelector);
        const traversal = initStyleContextMenuCallback(command, overlay, traversalIcon, 'Link Traversal Rules', initBasicBuildControls('Traversal', traversalEnumProperties, traversalEnumValues), containerSizingSelector, () => '');
