...
export function initGrid() {
    if (metadata.isEditor()) {
        ...
        dragger.dropWith(initContainmentDropCallback(command, initBiFilter(containment, ['cell'], ['table'])));
        dragger.dropWith(initContainmentDropCallback(command, initBiFilter(containment, ['connected', 'label'], ['cell'])));
        ...
        paletteContextMenu.add(initReplaceContextMenuCallback(palette, command, { keptAttributes: ['id', 'reference', 'end', 'style'] }, containment, initReplaceTemporaryCellChoiceSelector(), replaceTemporaryCellSelector, gridTemporaryReplaceStep, initGridTemporaryReplaceChecker()));
