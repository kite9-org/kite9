...
import com.kite9.k9server.adl.holder.pipeline.XMLBase;
...
public class Payload {
    ...
    public static final String ADL_META_NAMESPACE = "http://www.kite9.org/schema/adl-metadata";
    ...
    private static void insertMetadata(MetaRead adl, Document svgDocument) {
        ...
        if (hasMetadata) {
            ...
            // add new items
            firstMeta.setAttributeNS(XMLConstants.XMLNS_ATTRIBUTE_NS_URI, "xmlns:md", ADL_META_NAMESPACE);
    ...
    public static Document createMetaDocument(MetaRead m) {
        try {
            ...
            Element rootElement = doc.createElementNS(ADL_META_NAMESPACE, "md:metadata");
    ...
    private static Element createContent(Document svgDocument, String key, Object value) {
        Element entry = svgDocument.createElementNS(ADL_META_NAMESPACE, "md:"+key);
        if (value instanceof List) {
            ...
            Element id = svgDocument.createElementNS(ADL_META_NAMESPACE, "md:id");
            id.setTextContent(((UserMeta) value).getId());
            entry.appendChild(id);
            Element name = svgDocument.createElementNS(ADL_META_NAMESPACE, "md:name");
            name.setTextContent(((UserMeta) value).getDisplayName());
            entry.appendChild(name);
            Element icon = svgDocument.createElementNS(ADL_META_NAMESPACE, "md:icon");
            icon.setTextContent(((UserMeta) value).getIcon());
            entry.appendChild(icon);
            Element page = svgDocument.createElementNS(ADL_META_NAMESPACE, "md:page");
            page.setTextContent(((UserMeta) value).getPage());
            entry.appendChild(page);
            Element login = svgDocument.createElementNS(ADL_META_NAMESPACE, "md:login");
