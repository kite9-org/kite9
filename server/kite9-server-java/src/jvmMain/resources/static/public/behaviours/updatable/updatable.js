...
export function initMetadataBasedUpdater(command, metadata, transition) {
    ...
    if (processViaWebSocket) {
        ...
        delegate = initWebsocketUpdater(metadata.get("topic"), metadata.get("self"), resolver, contentType);
    }
    else {
        // not using web-sockets
        delegate = initHttpUpdater(metadata.get("self"), contentType, resolver);
    }
    return (update) => {
