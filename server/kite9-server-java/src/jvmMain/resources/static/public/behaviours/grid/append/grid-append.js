...
export function initCellAppendContextMenuCallback(command, selector = undefined) {
    ...
    function doAppend(container, selectedElements, side) {
        ...
        const keys = Object.keys(ordinalChangeMap).map(s => parseInt(s));
        ...
        if (e.length > 0) {
            const handleClick = () => {
                ...
                directions.forEach(s => {
                    cm.addControl(event, "/public/behaviours/grid/append/" + s.toLowerCase() + ".svg", "Append (" + s + ")", () => appendsCells(selector(), s, cm));
                });
            };
