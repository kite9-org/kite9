...
export const DRAGABLE_START_EVENT = 'dragable-start', DRAGABLE_MOVE_EVENT = 'dragable-move', DRAGABLE_END_EVENT = 'dragable-end', DRAGABLE_CANCELLED = 'dragable-cancel';
function defaultDragableSelector() {
    return Array.from(getMainSvg().querySelectorAll("[id][k9-ui~=drag]"));
}
export function initDragable(dragger, selector = undefined) {
 ...
 */
export function initDragableDragLocator(selector = undefined) {
    if (selector == undefined) {
        selector = function () {
 ...
 */
export function initDragContextMenuCallback(dragger, selector = null) {
    if (selector == undefined) {
        selector = function () {
