...
import com.kite9.k9server.adl.format.holder.pipeline.*;
...
public class ADLFactoryImpl implements ADLFactory {
	...
	abstract class XMLDomImpl extends XMLBaseImpl implements XMLDom<T> {
		
		private Document doc;
		
		public XMLDomImpl(URI uri, Map<String, List<String>> requestHeaders, Document doc, Map<String, Object> meta) {
			super(uri, requestHeaders, meta);
			this.doc = doc;
		}
	...
	class ADLDomImpl extends XMLDomImpl implements ADLDom {
		...
		public ADLDomImpl(URI uri, Map<String, List<String>> requestHeaders, Document doc, Map<String, Object> meta) {
			super(uri, requestHeaders, doc, meta);
		}
		
		@Override
		public <X extends DiagramFormat> ADLOutput<X> process(URI forLocation, X df) {
			try {
	...
	abstract class ADLOutputImpl<X extends DiagramFormat> extends XMLBaseImpl implements ADLOutput<X> {
		...
		public String getAsString() {
			if ((df instanceof DiagramFileFormat) && (!((DiagramFileFormat)df).isBinaryFormat())) {
