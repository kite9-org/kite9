...
public class BasicFormatSupplier implements FormatSupplier {
	...
	public BasicFormatSupplier(ADLFactory adlFactory, Cache c) {
		super();
		...
		mediaTypes = Arrays.stream(formats).flatMap(f -> Arrays.stream(f.getMediaTypes())).collect(Collectors.toList())
	...
	public Map<String, MediaType> getMediaTypeMap() {
		return Arrays.stream(formats).collect(Collectors.toMap(f -> f.getExtension(), f -> f.getMediaTypes()[0]));
	}
