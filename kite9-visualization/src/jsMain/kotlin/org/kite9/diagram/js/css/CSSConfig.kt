...
fun init() {
    ...
    register( CSSProperty(CSSConstants.ELEMENT_TYPE_PROPERTY, syntax<DiagramElementType>(), lower(DiagramElementType.UNSPECIFIED), false))
    register( CSSProperty(CSSConstants.ELEMENT_USAGE_PROPERTY, syntax<RectangularElementUsage>(), lower(RectangularElementUsage.REGULAR), false))
    register( CSSProperty(CSSConstants.LAYOUT_PROPERTY, syntax<Layout>()+" | none", "none", false))
    register( CSSProperty(CSSConstants.CONTENT_TRANSFORM,syntax<ContentTransform>(), lower(ContentTransform.NORMAL), false))

    // SIZING
    register( CSSProperty(CSSConstants.ELEMENT_HORIZONTAL_SIZING_PROPERTY,syntax<DiagramElementSizing>(), lower(DiagramElementSizing.MINIMIZE), false))
    register( CSSProperty(CSSConstants.ELEMENT_VERTICAL_SIZING_PROPERTY,syntax<DiagramElementSizing>(), lower(DiagramElementSizing.MINIMIZE), false))
...
//    registerCustomCSSShorthandManager(OccupiesShorthandManager())
    ...
    register( CSSProperty(CSSConstants.TRAVERSAL_PROPERTY,syntax<BorderTraversal>(), lower(BorderTraversal.LEAVING), false))
    register( CSSProperty(CSSConstants.TRAVERSAL_BOTTOM_PROPERTY,syntax<BorderTraversal>()+" | none", "none", false))
    register( CSSProperty(CSSConstants.TRAVERSAL_LEFT_PROPERTY,syntax<BorderTraversal>()+" | none", "none", false))
    register( CSSProperty(CSSConstants.TRAVERSAL_TOP_PROPERTY,syntax<BorderTraversal>()+" | none", "none", false))
    register( CSSProperty(CSSConstants.TRAVERSAL_RIGHT_PROPERTY,syntax<BorderTraversal>()+" | none", "none", false))

    // CONNECTION SIDES
    register( CSSProperty(CSSConstants.CONNECTIONS_PROPERTY,syntax<ConnectionsSeparation>(), lower(ConnectionsSeparation.SAME_SIDE), false))

    // ALIGNMENT
    register( CSSProperty(CSSConstants.VERTICAL_ALIGNMENT,syntax<VerticalAlignment>(), lower(VerticalAlignment.CENTER), false))
    register( CSSProperty(CSSConstants.HORIZONTAL_ALIGNMENT,syntax<HorizontalAlignment>(), lower(HorizontalAlignment.CENTER), false))

    register( CSSProperty(CSSConstants.VERTICAL_ALIGN_POSITION,"<length-percentage>", "50%", false))
    register( CSSProperty(CSSConstants.HORIZONTAL_ALIGN_POSITION,"<length-percentage>", "50%", false))
