...
public class Test59Sizing extends AbstractDisplayFunctionalTest {
	...
	public void test_59_10_TextAndBadge() throws Exception {
		renderDiagram(basicDiagram(
				glyphContainer(text("<svg:text>hello something</svg:text>", "font-size: 33px;") + badgeDecal(),
						"--kite9-padding: 10px;")));
	}
	...
	public void test_59_11_TextAndScaledDecal() throws Exception {
		renderDiagram(basicDiagram(
				glyphContainer(text("<svg:text>hello something else</svg:text>", "font-size: 25px;") + polyDecal(),
						"--kite9-padding: 10px;")));
	}
	...
	public void test_59_12_TestContainerMinimumSize() throws Exception {
		renderDiagram(basicDiagram(glyphContainer(
				text("<svg:text>hello something else</svg:text>", "font-size: 25px;")
						+ text("hello b", "font-size: 15px; --kite9-vertical-align: bottom;"),
				"--kite9-padding: 10px; --kite9-layout: down; --kite9-min-height: 120px")));
	}
	...
	public void test_59_14_TextAlign() throws Exception {
		renderDiagram(basicDiagram(glyphContainer(
				text("<svg:text>hello something else</svg:text>", "font-size: 25px; line-height: 120%;") +
						text("<svg:text>hello 2 left</svg:text>",
								"font-size: 15px; --kite9-text-bounds-width: 50px; --kite9-horizontal-align: left;")
						+
						text("<svg:text>middle align</svg:text>",
								"font-size: 15px;--kite9-vertical-align: center; --kite9-horizontal-align: center; text-align: center; --kite9-text-bounds-width: 50px; ")
						+
						text("<svg:text>right align</svg:text>",
								"font-size: 15px; --kite9-vertical-align: bottom; --kite9-horizontal-align: right; text-align: right; --kite9-text-bounds-width: 50px;")
						+
						text("<svg:text>hello this is some fully\nall over the place happening\njustified bottom</svg:text>",
								"font-size: 15px; --kite9-vertical-align: bottom; text-align: justify; --kite9-text-bounds-width: 3000px;"),
	...
	private String textWithBaseline(String text) {
		return text("<svg:text>" + text + "</svg:text>",
				"font-size: 25px; --kite9-vertical-align: bottom; --kite9-padding: 0px;");
	}
				...
				glyphContainer(
						glyphContainer(
								...
								glyphContainer(
										...
										"--kite9-padding: 00px; --kite9-layout: down;"),
	...
	public void test_59_18_TestTextBoundedSize() throws Exception {
		renderDiagram(basicDiagram(glyphContainer(
				text("<svg:text>hello something else</svg:text>",
						"font-size: 25px; --kite9-text-bounds-size: 20px 500px; ")
						+ text("<svg:text>hello b this could be a long bit of text</svg:text>",
								"font-size: 15px; --kite9-vertical-align: bottom; --kite9-text-bounds-size: 100px 500px;"),
				"--kite9-padding: 0px; --kite9-layout: down; --kite9-min-height: 120px")));
	}
	...
	public void test_59_19_TestWrap() throws Exception {
		renderDiagram(basicDiagram(
				...
				glyphContainer(
						text("<svg:text>a b c d e f g h i j k l m n o p q r s t u v w x y z</svg:text>",
								"font-family: opensans-bold-webfont; font-size: 25px; --kite9-text-bounds-size: 200px 100px; --kite9-margin: 0; "),
						"--kite9-padding: 0px; --kite9-layout: down; ")
						+ glyphContainer(
								text("<svg:text>a b c d e f g h i j k l m n o p q r s t u v w x y z</svg:text>",
										"font-family: opensans-light-webfont; font-size: 25px; --kite9-text-bounds-size: 200px 100px; --kite9-margin: 0; "),
								"--kite9-padding: 0px; --kite9-layout: down; ")));
	}
	...
	private String textWithTransform(String xml, String extraAtts, String transform) {
		return "      <svg:g style=\"--kite9-type: svg; font-family:  opensans-light-webfont; " + extraAtts
	...
	private String basicDiagram(String xml) throws Exception {
		return "\n  <svg:svg xmlns:xlink=\"http://www.w3.org/1999/xlink\" style=\"--kite9-type: diagram; --kite9-layout: down; \" "
				...
				"<svg:defs>" +
				"<svg:style> @import url(\""
				+ Test59Sizing.class.getResource("/stylesheets/tester.css").toURI().toString() + "\");</svg:style>" +
