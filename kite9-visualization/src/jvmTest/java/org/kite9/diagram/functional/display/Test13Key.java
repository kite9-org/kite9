...
public class Test13Key extends AbstractDisplayFunctionalTest {
	...
	public void test_13_8_separateGlyphsWithKey() throws Exception {
		...
		Glyph rs = new Glyph("RS", "Director", "Ridley Scott",
				createList(new TextLine("Directed: Thelma & Louise"), new TextLine("Directed: Gladiator")),
				createList(s));
		DiagramKite9XMLElement d1 = new DiagramKite9XMLElement("my_diagram", listOf(hf, rs),
				new Key("Big Movies", "", createList(s)));
		renderDiagram(d1);
	}
	...
	public void test_13_11_AllTextAndWith1Symbol() throws Exception {
		...
		Key k = new Key("some bold text", "some body text\nspanning two lines",
				createList(new Symbol("Some information", 'S', SymbolShape.CIRCLE)));
	...
	public void test_13_13_ThreeColumns() throws Exception {
		...
		Key k = new Key(
				"some bold text taking lots of space dsjhksdjfhjkdskj d dcdshf kdskjfhjdsf jkhdskjf skdjhfjkds j s",
				null, createNarrowSymbolList(7));
