...
public class Test29ContainerSizing extends AbstractDisplayFunctionalTest {
	...
	public void test_29_2_ContainerSizes() throws Exception {
		...
		DiagramKite9XMLElement d1 = new DiagramKite9XMLElement(HelpMethods.listOf(c1, c2, c3, c4),
				new Key("Container positions enforced by links", "", new ArrayList<>()));
		renderDiagram(d1);
	...
	public void test_29_3_LikeASequence() throws Exception {
		...
		DiagramKite9XMLElement d1 = new DiagramKite9XMLElement(HelpMethods.listOf(c1, c2, c3, c4),
				new Key("Right layout and links go right", "", new ArrayList<>()));
	...
	public void test_29_4_CornersInContainers() throws Exception {
		...
		DiagramKite9XMLElement d1 = new DiagramKite9XMLElement(HelpMethods.listOf(c1, c2, c3, c4),
				new Key("Container positions enforced by two links", "", new ArrayList<>()));
	...
	public void middleGuyTopology(boolean directedEdges, boolean labels, Layout l, int directedContainers, String k)
			throws Exception {
		...
		Context top = new Context("top", tops, true, new TextLabel("top"),
				directedContainers > 0 ? Layout.RIGHT : null);
		Context bottom = new Context("bottom", bottoms, true, new TextLabel("bottom"),
				directedContainers > 1 ? Layout.RIGHT : null);
		Context left = new Context("left", lefts, true, new TextLabel("left"),
				directedContainers > 2 ? Layout.DOWN : null);
		Context right = new Context("right", rights, true, new TextLabel("right"),
				directedContainers > 3 ? Layout.DOWN : null);
		// right.setAttribute("style", CSSConstants.ELEMENT_VERTICAL_SIZING_PROPERTY+":
		// maximize; ");

		for (int i = 0; i < 3; i++) {
			if (i == 1) {
				if (directedEdges && labels) {
					new Link(middle, tops.get(i), null, new TextLabel("straight" + i), null,
							new TextLabel("straight" + i), Direction.UP);
					new Link(middle, bottoms.get(i), null, new TextLabel("straight" + i), null,
							new TextLabel("straight" + i), Direction.DOWN);
					new Link(middle, lefts.get(i), null, new TextLabel("straight" + i), null,
							new TextLabel("straight" + i), Direction.LEFT);
					new Link(middle, rights.get(i), null, new TextLabel("straight" + i), null,
							new TextLabel("straight" + i), Direction.RIGHT);
		...
		DiagramKite9XMLElement d1 = new DiagramKite9XMLElement("dia", HelpMethods.listOf(uber), l,
				new Key(k, "", new ArrayList<>()));
