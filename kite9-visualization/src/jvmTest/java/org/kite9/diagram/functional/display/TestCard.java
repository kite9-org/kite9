...
public class TestCard extends AbstractDisplayFunctionalTest {
	...
	public DiagramKite9XMLElement createTestCard() {
		...
		Glyph withSymbols = new Glyph("withSymbols", null, "With Symbols", null,
		...
		Glyph fullGlyph = new Glyph("fullGlyph", "Some complex stereotype", "Full Glyph", HelpMethods.createList(
				...
				new TextLine("Another text line")), HelpMethods.createList(aDia, QCir, gHex));

		LinkBody a1 = new LinkBody("a1", "Arrow with a label");

		LinkBody a2 = new LinkBody("a2", null);

		Context occContext = new Context(HelpMethods.listOf(simple, withStereo), true,
				new TextLabel("Multiline label\nfor this context"), null);
		...
		Key k = new Key("This is a test card",
				"It demonstrates all the attr in the ADL\nlanguage to allow you to check rendering",
				HelpMethods.createList(aDia, QCir, gHex, AHex, OCir, MDia, oCir));
		...
		DiagramKite9XMLElement d = new DiagramKite9XMLElement(
				HelpMethods.listOf(emptyContext, otherContext, a2, withSymbols, text1, other), k);
		return d;
	}
	...
	/**
	 * Disabling some checks. 61_12 edges don't run straight anymore, however it's
	 * correct behaviour.
	 * 
	 * @return
	 */
