...
public class Test54SVGPrimitives extends AbstractDisplayFunctionalTest {
	...
	public void test_54_2_FixedGraphicsPrimitive() throws Exception {
		String someXML = svgOpen() + diagramOpen() + fixedSizeOpen() + svgSquiggle() + fixedSizeClose() + diagramClose()
				+ svgClose();
		transcodeSVG(someXML);
	}
	...
	/**
	 * Important thing in this test is that it doesn't crash in the presence of a
	 * broken image
	 */
	@Test
	...
	private String svgOpen() {
		return "<svg:svg xmlns:xlink='http://www.w3.org/1999/xlink' xmlns:svg='" + Kite9Namespaces.SVG_NAMESPACE
	...
	private String diagramOpen() {
		return "<svg:g xmlns='" + Kite9Namespaces.ADL_NAMESPACE
				+ "' id='one' style='--kite9-type: diagram; --kite9-padding: 50px;'><svg:g style='fill: white; stroke: grey; stroke-width: 3px; '>";
	}
	...
	private String containerOpen(String id, String fill) {
		return "<svg:g id='" + id
				+ "' style='--kite9-type: container; --kite9-sizing: minimize; --kite9-padding: 10px; --kite9-margin: 10px;  '><svg:g style='fill: "
				+ fill + "; '>";
	}
