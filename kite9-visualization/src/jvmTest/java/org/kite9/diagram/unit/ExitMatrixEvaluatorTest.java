...
public class ExitMatrixEvaluatorTest extends AbstractTest {
	...
	public void testExtraDistance() {
		...
		Assert.assertEquals(.5f,
				(float) new ExitMatrixEvaluator().calculateExtraExternalLinkDistance(left, right, Layout.LEFT, rh),
				0.01f);
		// right (now bottom) matrix up side gets cost of .5 each
		Assert.assertEquals(1.5f,
				(float) new ExitMatrixEvaluator().calculateExtraExternalLinkDistance(left, right, Layout.UP, rh),
				0.01f);
	}
	...
	/**
	 * In this case, the narrow should be on the bottom, otherwise there will be too
	 * much occlusion
	 */
	@Test
	public void testWideAndNarrowSpans() {
		...
		Assert.assertEquals(.5f,
				(float) new ExitMatrixEvaluator().calculateExtraExternalLinkDistance(wide, narrow, Layout.UP, rh),
				0.01f);
		// single narrow link going further
		Assert.assertEquals(1f,
				(float) new ExitMatrixEvaluator().calculateExtraExternalLinkDistance(wide, narrow, Layout.DOWN, rh),
				0.01f);
	...
	public void testWideAndNarrowSpansOffset() {
		...
		// only half of top is obscured, by 1/3 of the other. 1/2 * 1 * 1/2 multiplier =
		// .25
