...
abstract class AbstractDiagramElementFactory<X>(val failOnUnspecified: Boolean = true) :
        DiagramElementFactory<X> {
                ...
                when (lt) {
                    DiagramElementType.DIAGRAM -> {
                        if (parent != null) {
                            throw context!!.contextualException(
                                    "Can't nest type 'diagram' @ " + getId(el),
                                    el
                            )
                        }
                        DiagramImpl(
                    ...
                    DiagramElementType.CONTAINER ->
                            when (usage) {
                                ...
                                RectangularElementUsage.DECAL ->
                                        throw context!!.contextualException(
                                                "Decal containers not supported yet: @" + getId(el),
                                                el
                                        )
                            }
                    ...
                    DiagramElementType.UNSPECIFIED ->
                            if (failOnUnspecified) {
                                throw context!!.contextualException(
                                        "Don't know how to process element: " +
                                                el +
                                                "(" +
                                                el.tagName +
                                                ") with type " +
