...
/**
 * Represents the relative positions of two other groups within the diagram, allowing the immediate
...
abstract class AbstractCompoundGroup(
    ...
    /**
     * This will process [OrderingTemporaryConnection]s first, so that if there is a contradiction
     * in the links, it will occur on one of the Link - Connections.
     */
    fun fileLinks(linksGroup: Group, toGroup: Group): LinkManager.LinkDetail? {
        ...
        linksGroup.processAllLeavingLinks(
                true,
                linkManager.allMask(),
                object : LinkManager.LinkProcessor {
                    override fun process(
                            originatingGroup: Group,
                            destinationGroup: Group,
                            ld: LinkManager.LinkDetail
                    ) {
                        val ld2 = fileLink(linksGroup, destinationGroup, toGroup, ld)
                        if (ld2 != null) {
                            ldOut.add(ld2!!)
                        }
                    }
    ...
    private fun fileLink(
            from: Group,
            to: Group,
            merging: Group,
            ld: LinkManager.LinkDetail
    ): LinkManager.LinkDetail? {
