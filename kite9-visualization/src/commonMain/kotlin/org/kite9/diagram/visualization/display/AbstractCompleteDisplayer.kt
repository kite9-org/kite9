...
abstract class AbstractCompleteDisplayer(buffer: Boolean) :
    ...
    override fun getMinimumDistanceBetween(
        ...
        val distance = getMinimumDistanceInner(a, aSide, b, bSide, direction, along, concave)
        log.send(
                if (log.go()) null
                else "Minimum distances between  $a  $aSide $b $bSide in $direction is $distance"
        )
        return max(distance, buffer)
    }

    /**
     * Given two diagram attr, separated with a gutter, figure out how much space must be between
     * them.
     * @param concave
     */
    private fun getMinimumDistanceInner(
        ...
        length =
                if (a is Connection && b is Connection) {
                    ...
                    getMinimumDistanceRectangularToRectangular(
                            a,
                            aSide,
                            b,
                            bSide,
                    ...
                    getMinimumDistanceRectangularToConnection(a, aSide, b, bSide, direction, along)
    ...
    private fun getMinimumDistanceRectangularToConnection(
        ...
        return if (aSide === direction) {
            ...
            if (isEventualParent(a, b)) {
                ...
                incorporateAlongMinimumLength(along, direction, length, a, aSide, b, bSide)
            }
        } else {
            if (a is ConnectedRectangular && b.meets(a)) {
                // a connection arriving at a rectangular
                incorporateAlongMinimumLength(along, direction, 0.0, a, aSide, b, bSide)
            } else {
                // we are inside a, so use the padding distance
                val inset = getPadding(a, direction)
                val margin = getMargin(b, reverse(direction)!!)
                val length = max(inset, margin)
                incorporateAlongMinimumLength(along, direction, length, a, aSide, b, bSide)
            }
        }
    ...
    private fun incorporateAlongMinimumLength(
        ...
        return if (along is Connection) {
            ...
            val alongDist = getAlongMinimumLength(along, direction, a, aSide, b, bSide)
            max(`in`, alongDist)
    ...
    private fun getAlongMinimumLength(
        ...
        return if (along === a && b is Connection) {
            ...
            val startA = if (a.meets(along)) a.getDecorationForEnd(along) else null
            val startB = if (b.meets(along)) b.getDecorationForEnd(along) else null
    ...
    private fun portPositionPixels(p: Port, d: Direction, on: Rectangular): Double {
        ...
        val pxDist: Double =
                when {
                    p.getContainerPosition().type == Measurement.PERCENTAGE ->
                            elementLength * p.getContainerPosition().amount / 100
                    p.getContainerPosition().amount < 0 ->
                            elementLength + p.getContainerPosition().amount
    ...
    private fun getPortFor(
            a: ConnectedRectangular,
            c1: DiagramElement?,
            favourFrom: Boolean
    ): Port? {
        if (c1 is Connection) {
            val order: List<Port?> =
                    if (favourFrom) listOf(c1.getFrom() as? Port, c1.getTo() as? Port)
                    else listOf(c1.getTo() as? Port, c1.getFrom() as? Port)

            val first = order.filterNotNull().filter { it.getContainer() == a }.firstOrNull()
    ...
    private fun getInternalDistance(
            a: DiagramElement?,
            aSide: Direction?,
            bSide: Direction?
    ): Double {
     ...
     * being considered. This should include terminators.
     */
    protected abstract fun getLinkMinimumLength(
            element: Connection,
            starting: Boolean,
            ending: Boolean
    ): Double
    ...
    /**
     * Distance from the edge of a connected element to the connection, minimum. (Could be increased
     * by terminators)
     */
    ...
    /**
     * Set buffer > 0 to ensure gaps even around invisible attr. 0 should be used for correct
     * rendering
     */
    init {
