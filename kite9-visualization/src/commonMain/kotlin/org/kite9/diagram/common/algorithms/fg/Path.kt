...
class Path : PathLocation<Path> {
    ...
    constructor(p: Path, a: Arc, reversed: Boolean) {
        ...
        cost += p.getCost() + if (reversed) a.getIncrementalCost(-1) else a.getIncrementalCost(+1)
    ...
    override fun toString(): String {
        return (if (route == null) "" else (if (reversed) "!" else "") + route.toString()) +
                (if (nextPathItem == null) "" else "/" + nextPathItem.toString()) +
                ":" +
                cost
    }
    ...
    override operator fun compareTo(other: Path): Int {
        return if (getCost() > other.getCost()) {
            1
