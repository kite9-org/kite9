...
abstract class DirectedEdgePlanarizationBuilder(em: ElementMapper, gp: GridPositioner) : MGTPlanarizationBuilder(em, gp) {
    ...
    private fun getInsertionDirection(layoutDirection: Layout?, from: Vertex, to: Vertex): Direction {
        when (layoutDirection) {
            Layout.LEFT, Layout.RIGHT, Layout.HORIZONTAL -> return if (from.routingInfo!!.centerX() < to.routingInfo!!.centerX()) Direction.RIGHT else Direction.LEFT
            Layout.UP, Layout.DOWN, Layout.VERTICAL -> return if (from.routingInfo!!.centerY() < to.routingInfo!!.centerY()) Direction.DOWN else Direction.UP
        }
        throw LogicException("Couldn't determine direction to insert in")
    }
