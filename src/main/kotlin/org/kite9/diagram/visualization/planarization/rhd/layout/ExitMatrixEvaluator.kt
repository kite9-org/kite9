...
object ExitMatrixEvaluator {
	...
	fun calculateExtraExternalLinkDistance(
        ...
        return calcEEDistanceOneSide(aMatrix, bMatrix.getSizeInDirectionOfLayout(aDirection), aDirection) +
                calcEEDistanceOneSide(bMatrix, aMatrix.getSizeInDirectionOfLayout(aDirection), reverse(aDirection))
    }

    private fun calcEEDistanceOneSide(aMatrix: ExitMatrix, distance: Bounds, d: Layout?): Double {
        val count = aMatrix.getLinkCount(d, RelativeSide.OPPOSITE, -1) +
...
//						"\n\tto "+((LeafGroup)to).getContained()+" at "+bRI);
    ...
    private fun addOneOccludedSide(
        aMatrix: ExitMatrix, bMatrix: ExitMatrix, ad: Layout?,
        rh: RoutableHandler2D
    ): Float {
