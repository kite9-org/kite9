...
/**
 * Allows us to wrap all XML elements into GraphicsNodes, so that the entire XML structure is preserved.
 * 
 * @author robmoffat
 *
 */
public class PreservingGVTBuilder extends GVTBuilder {
	
	public Bridge getBridge(BridgeContext ctx, String namespaceURI, String localName) {
		Bridge inner = ctx.getBridge(namespaceURI, localName);
		if (inner instanceof GraphicsNodeBridge) {
			return inner;
		} else {
			return new PlaceholderGraphicsNodeBridge((GenericBridge) inner, namespaceURI, localName) ;
		} 
	}
	
    public Bridge getBridge(BridgeContext ctx, Element element) {
    	if (element == null) {
            return null;
        }
        ...
        return getBridge(ctx, namespaceURI, localName);
    }
     ...
     * the GVT tree
     */
    public GraphicsNode build(Kite9BridgeContext ctx, Document document) {
        ...
        try {
            ...
            Bridge bridge = getBridge(ctx, svgElement);
