...
public class ConnectionEdgeRouteFinder2 extends AbstractRouteFinder {
	...
	public ConnectionEdgeRouteFinder2(MGTPlanarization p, RoutableReader rh, Edge ci, ElementMapper em, Direction edgeDir, CrossingType it, GeographyType gt) {
		super(p, rh, getEndZone(rh, ci), getExpensiveAxis(ci, gt), getBoundedAxis(ci, gt), ci);
		this.startZone = getStartZone(rh, ci);
...
//	}
	...
	private static RoutingInfo getEndZone(RoutableReader rh, Edge ci) {
		if (ci.getTo() instanceof ContainerVertex) {
			DiagramElement und = ci.getOriginalUnderlying();
			if (und instanceof Connection) {
				Connected to = ci.isReversed() ? ((Connection) und).getFrom() : ((Connection) und).getTo();
				return rh.getPlacedPosition(to);
			}
		} 
		
		return ci.getTo().getRoutingInfo();
	}
	
	private static RoutingInfo getStartZone(RoutableReader rh, Edge ci) {
		if (ci.getFrom() instanceof ContainerVertex) {
			DiagramElement und = ci.getOriginalUnderlying();
			if (und instanceof Connection) {
				Connected from = ci.isReversed() ? ((Connection) und).getTo() : ((Connection) und).getFrom();
				return rh.getPlacedPosition(from);
			}
		} 
		
		return ci.getFrom().getRoutingInfo();
	}

	private RoutingInfo getCompleteZone(Edge ci) {
		RoutingInfo from = getStartZone(rh, ci);
		RoutingInfo to = getEndZone(rh, ci);
	...
	protected void createInitialPaths(State<LocatedEdgePath> pq) {
		...
		if (from instanceof ContainerVertex) {
			...
			if (und instanceof Connection) {
				Connected from2 = ((Connection) und).getFrom();
				
				if (from2 instanceof Container) {
					Container c = (Container) from2;
