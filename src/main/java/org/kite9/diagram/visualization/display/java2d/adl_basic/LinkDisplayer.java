...
public class LinkDisplayer extends AbstractRouteDisplayer implements ComponentDisplayer {
	...
	public LinkDisplayer(CompleteDisplayer parent,Stylesheet ss, Graphics2D g2, boolean shadow, int xo, int yo) {
	...
	public class StyledEndDisplayer extends AbstractPerimeterAwareEndDisplayer {
		...
		public int draw(int x, int y, Direction d, Graphics2D g2, Paint lc, Paint fc) {
			if (s != null) {
		...
		@Override
		public void draw(Graphics2D gp, Paint lc, Paint fc) {
	...
	public void draw(DiagramElement al, RenderingInformation r) {
		...
		if ((rr.size()==0) && ((rr.getPerimeterPath()==null) || (rr.getPerimeterPath().length()==0))) {
