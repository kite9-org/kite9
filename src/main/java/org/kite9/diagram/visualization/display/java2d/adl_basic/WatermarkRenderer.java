...
public class WatermarkRenderer {
	...
	public static final int WATERMARK_LAYER = 2;
	public static final int COPYRIGHT_LAYER = 3;
	...
	public void display(GraphicsSourceRenderer<?> gs, Dimension2D size, Stylesheet ss) {
		displayWatermark(gs.getGraphicsLayer(WATERMARK_LAYER, (float) .15, size), size, ss);
		...
		displayCopyright(gs.getGraphicsLayer(COPYRIGHT_LAYER, (float) 1, size), size, ss); 
	}
	
	public void displayCopyright(Graphics2D g2, Dimension2D size, Stylesheet ss) {
		...
		double widthNorm = fontMetrics.getStringBounds(COPYRIGHT_NORMAL, g2).getWidth();
		double widthShort = fontMetrics.getStringBounds(COPYRIGHT_SHORT, g2).getWidth();
	...
	public void displayWatermark(Graphics2D g2, Dimension2D size, Stylesheet ss) {
		try {
			AffineTransform at = g2.getTransform();
			g2.setTransform(new AffineTransform());
