...
public class Segment implements Comparable<Segment> {
	...
	public boolean hasUnderlyingWithId(String id) {
		return underlyings.stream()
				.map(u -> u.getDiagramElement())
	 ...
	 */
	public int getAdjoiningSegmentBalance() {
		boolean isHorizontal = dimension == Dimension.H;
		int balance = getVerticesInSegment().stream()
			.mapToInt(v -> isHorizontal ?
				dartCount(v, Direction.DOWN) - dartCount(v, Direction.UP) :
				dartCount(v, Direction.RIGHT) - dartCount(v, Direction.LEFT))
			.sum();
		return balance;					
	}

	private int dartCount(Vertex v, Direction d) {
		for (Edge e : v.getEdges()) {
